...
function State04() {
  // & === Hooks === //
  // 1) 폼 데이터 상태 관리
  const [formData, setFromData] = useState<FormData>(initialFromData);

  // 2) 폼 입력 오류 메시지 상태 관리
  const [error, setError] = useState<FormData>(initialFromData);
  ...
  const handleSignUpSubmit = (e:React.FormEvent) => {
    ...
    if (!confirmPassword.trim) {
      tempError.confirmPassword = '비밀번호 확인을 입력해주새요.';
      isValid = false;
    }

    if (!email.trim) {
      tempError.email = '이메일을 입력해주새요.';
      isValid = false;
    }
    ...
    >
      ...
      <form onSubmit={handleSignUpSubmit}>
        <div>
          ...
          {/* 
            p태그 외부 소괄호 생략
            : JSX 안에서 한 줄짜리 표션식은 ()소괄호로 안 감싸도 됨
          */}
