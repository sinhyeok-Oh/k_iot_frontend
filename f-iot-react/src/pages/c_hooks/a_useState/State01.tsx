...
function State01() {
  // ? 기본 구조와 명명 규칙
  // const [state, setState] = useState<type>(intialValue);

  // % state        : 현재 상태값 (변수)
  // % setState     : 상태를 업데이트하는 함수
  // % initialValue : 초기밧, 초기 상태를 설정 (생략 가능 -undefined 값)
  ...
  // === HOOKS (useState) === //
  //? useState 실습 예제 - 가운터 컴포넌트
  const [count, setCount] = useState<number>(0);
  const [massage, setMessage] = useState<string>('안녕하세요');
  // if (true) {
  ...
  const handleUpClick = () => {
    // setCount(5); //  상태 설정 함수의 인자값의 상태 변수로 전달
    // setCount(count + 1); // 0 + 1 === 1
    // setCount(count + 1); // 0 + 1 === 1

    // 2) 함수형 컴포넌트 사용
    //    : 현재(이전, 최신)의 상태값을 기반으로 상태를 업에이트 하는 경우 사용
    // cf) set- 상태 변경 함수 내에서 콜백 함수
    //    setCount(() => {})
    //    >> 해당 콜백 함수의 인자는 '상태의 최신값'
    //    >> prev-상태명 (prevName, prevCount, prevUesr / previous 이전의)
    setCount(prevCount => prevCount + 1); // 0 + 1 === 1
    setCount(prevCount => prevCount + 1); // o + 1 === 2
  }
