...
/*
  ...
  2) 무한 렌더링 루프
    : useEffect 안에서 state를 잘못 업데이트하면 무한 렌더링 발행
  3) 비동기 함수 직접 전달 금지
    : useEffect(async() => {...}) - X
...
// ! === 푸드트럭 에약 시스템 === // 
// : 푸드트럭 예약 괸리 화면에서, 관리자가 고객의 예약 상태를 실시간으로 모니터링하고 승인
/*
  1) 트럭 예약 데이터 불러오기 
    - API 호출
    - 마운트 시 실행
  ...
  2) 일정 주기 자동 새로고침
    ...
    ? 폴링(Polling)
      : 클라이언트가 서버에게 주기적으로 요청을 보내 새로운 데이터나 상태 변환를 확인하는 방식
...
// : 실제 서비스에서는 "~/api/v1/reservation" REST API 사용
...
function Effect_Practice01() {
  ...
  // === Function === //
  // 예약 데어터 가져오기 함수 정의 (비동기)
  const fetchReservations = async () => {
    setLoading(true);   // 로딩 시작
    setError(null);     // 에러 추기화
    ...
    try {
      ...
      const mappedData: Reservation[] = data.slice(0.5).map((item: any, idx: number) => ({
        ...
        truck_name: `Truck-${idx + 1}`,                  // 트럭 일름 예시
  ...
  // ! 4. 로딩 / 에러 / 성공 분기 렌더링
  if (loading) return <p> 🔄️예약 정보를 불러오는 중입니다...</p>
          ...
          <li key={reservation.id} style={{
            ...
            <div style={{ marginTop: '8px' }}>
              ...
              {reservation.status !== 'CONFIRMEND' && (
                <button onClick={() => updateReservationStatus(reservation.id, "CONFIRMEND")}>승인
                </button>
              )}

              {/* CANCELLED가 아닐 때만 취소 버튼 */}
              {reservation.status !== 'CANCELLED' && (
                <button onClick={() => updateReservationStatus(reservation.id, "CANCELLED")}>취소
                </button>
              )}
            </div>
          </li>
