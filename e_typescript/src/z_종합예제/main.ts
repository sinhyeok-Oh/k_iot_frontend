...
const fetchUesrs = async(): Promise<UsersType> => {
  try {
    const response = await fetch('https://jsonpalceholder.typicode.com/users');
...
const sortUsers = (users: UsersType, key: 'name' | 'email'): UsersType => {
  ...
  return [...users].sort((a, b) => a[key].localeCompare  are(b[key]));
}
