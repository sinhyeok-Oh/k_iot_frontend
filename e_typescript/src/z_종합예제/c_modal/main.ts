...
const fetchUsers = async(): Promise<UserType> => {
  try {
    const response = await fetch('https://jsonpalaceholder.typicode.com/users');
...
const addEventListeners = (users: UserType) => {
  const userList = document.getElementById('user-list');

  if (userList) {
    userList.addEventListener('click', (e) => {
      //? cf) targer VS currentTarget
      ...
      // : 이벤트 발생한 요소와 가장 가까운 .user-card 요소를 반환
  ...
  if (modal && closeModal) {
    ...
    // cf) document: window 로드되는 HTML 문서 그자체
