...
! 2) GradeManagement 클래스
  ...
  - 조건에 따른 학생 필터링 & 정렬
    > getTopStudents(threshold)
      : 편균 성적이 주어진 값(한계점) 이상인 학생을 필러팅 + 내림차순 정렬 반환
    > filter(), sort()
...
class Student {
  ...
  getAverageScore() {
    // Object.values(객체);
    // >> 객체된 객체가 가지는 속성의 값들로만 이루어진 배열을 반환
...
class GradeManegement {
  ...
  // 모든 학생의 평균 성적 배열 반환
  // 반환 형태는: [{ id, name, average }, { id, name, average }, ...]
  getAverageScore() {
...
// ! == 프로그램 실행 == //
const gradeSystem = new GradeManegement();
