...
class Book {
  ...
  returnBook() {
    if (!this.isAvailable) {
      ...
      this.isAvailable = true;
      console.log(`${this.isAvailable}이(가) 성공적으로 반납되었습니다.`);
    }
  }
}
...
filterBooksByTitle(keyword) {
  // 포함되는 제목을 필터링
  // >> 전체 목록 순회 + 각 데이터의 title 값에 매개변수의 keyword 값이 포함(includes)된 경우 새로운 배열로 반환
  // >> 해당 배열전체 출력
  //  : filter + forEach
...
filterBookByAvailable(isAvailable) {
  // +) isAvailable 값에 따라 true면 출력 시 ${대여 가능}인 책 목록
  //        , false면 출력시 ${대여 중}인 책 목록
  const status = isAvailable ? '대여 간으' : '대여 중';
...
busanLibrary.addBook('자바스크립트 공부는 노잼', '이승아');
busanLibrary.addBook('자바 공부는 노잼', '이도경');
busanLibrary.addBook('SQLD 공부는 노잼', '이지회');
busanLibrary.addBook('타입스크립트 공부는 노잼', '이지훈');
busanLibrary.addBook('리엑트 공부는 노잼', '이승아');
...
busanLibrary.updateBook(2, 'Java 공부는 노잼', null);
busanLibrary.updateBook(2, 'Java study is fun', 'lee do kyeong');
...
yangsanLibrary.addBook('자바스크립트 공부는 노잼', '이승아');
yangsanLibrary.addBook('자바 공부는 노잼', '이도경');
yangsanLibrary.addBook('SQLD 공부는 노잼', '이지회');
yangsanLibrary.addBook('타입스크립트 공부는 노잼', '이지훈');
yangsanLibrary.addBook('리엑트 공부는 노잼', '이승아');
...
yangsanLibrary.filterBookByAvailable(true);
yangsanLibrary.filterBookByAvailable(false);
